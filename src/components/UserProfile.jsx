...
const GRADES = {
  bronze: { label: '브론즈', emoji: '🥉', color: '#CD7F32', minPoints: 0 },
  silver: { label: '실버', emoji: '🥈', color: '#C0C0C0', minPoints: 100 },
  gold: { label: '골드', emoji: '🥇', color: '#FFD700', minPoints: 300 },
};
...
function UserProfile({ user, onClose }) {
  ...
  const progress = nextGrade
    ? Math.round((points / nextGrade.needed + points) * 100)
    : 100;
          ...
          <>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{
                  ...
                  color: grade.color, marginTop: '2px'
                }}>
                  {grade.label} 등급
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
              {[
                ...
                { label: '제보 횟수', value: `${reportCount}회`, emoji: '📝', color: '#2ED573' },
              ].map(stat => (
            ...
            <div style={{ fontSize: '12px', color: '#999', lineHeight: 1.6 }}>
              💡 재고 제보 +10P · 투표 +2P · 100P = 실버 · 300P = 골드
            </div>
          </>
        )}
      </div>
